...
function WatchPage({ video, videos, chapters, onBack, onNext, onSelect, onProgress }: {
  ...
  function save(force = false) {
    ...
    if (new URL(player.currentSrc || player.src, location.href).pathname !== `/media/${video.id}.mp4`) return;
    const now = Date.now();
